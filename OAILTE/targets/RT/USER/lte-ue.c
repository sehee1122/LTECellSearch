...
int find_bandFromDLfreq(uint32_t DLfreq, uint32_t* ULfreq_offset)
{
	int ind, i;
	int current_band = -1;
    ...
    for ( ind=0; ind < sizeof(eutra_bands) / sizeof(eutra_bands[0]); ind++) 
	{
        ...
        if ( eutra_bands[ind].dl_min <= DLfreq && eutra_bands[ind].dl_max >= DLfreq ) 
		{
            for (i=0; i<4; i++)
            {
               	ULfreq_offset[i] = eutra_bands[ind].ul_min - eutra_bands[ind].dl_min;
           	}
            break;
        }
		current_band = -1;
...
void config_txRxFreq(PHY_VARS_UE *UE, uint32_t* DLfreq, uint32_t* ULfreqOffset, int freq_offset, int value_autocal)
{
	int i;
	
    for (i=0; i<openair0_cfg[UE->rf_map.card].rx_num_channels; i++) 
	{
        openair0_cfg[UE->rf_map.card].rx_freq[UE->rf_map.chain+i] = DLfreq[i]+freq_offset;
        openair0_cfg[UE->rf_map.card].tx_freq[UE->rf_map.chain+i] = DLfreq[i]+ULfreqOffset[i]+freq_offset;
        openair0_cfg[UE->rf_map.card].autocal[UE->rf_map.chain+i] = value_autocal;
...
static void *UE_thread_synch(void *arg) {
	...
	//initial configuratio of the synchronization thread ---------------------------------------
    ...
    init_thread(100000, 500000, FIFO_PRIORITY-1, &cpuset, threadname); //configuring thread parameters : cpu affinity and priority

	//flag initialization
    UE->is_synchronized = 0; //indicating whether UE is synchronized or not (This is used in RF code : after synchronized, RF reads IQ values in TXRX buffer


	//initialize frequency band parameters -----------------------------------------------------
    if (UE->UE_scan == 0) //if Carrier frequency band option is given (carrier frequency band is given)
	{
		AssertFatal((current_band = find_bandFromDLfreq(downlink_frequency[0][0], uplink_frequency_offset[CC_id])) >= 0, 
			"Can't find EUTRA band for frequency");
		LOG_I(PHY, "Scanning band %d, dl_min %"PRIu32", ul_min %"PRIu32"\n", current_band, eutra_bands[current_band].dl_min,eutra_bands[current_band].ul_min);

		//configuration of USRP frequency parameter
		config_txRxFreq(UE, downlink_frequency[CC_id], uplink_frequency_offset[CC_id], 0, 1);
    }
	else 
	{
        current_band=0;
		...
		config_txRxFreq(UE, downlink_frequency[CC_id], uplink_frequency_offset[CC_id], 0, UE->rx_total_gain_dB);
    }
	

	
	//synch. thread starts ----------------------------------------------------------------------
    while (oai_exit==0) //thread loop 
	{
		// waiting IQ values come in through USRP (RF) --------------------------
		//access to common resource (MUTEX) : synch instance
        ...
        LOG_I(PHY, "[UE thread Synch] Running Initial Synch (mode %d, scan carrier : %d)\n", UE->mode, UE->UE_scan_carrier);

        if ((ret = initial_sync( UE, UE->mode )) == 0) 
		{ // if sync is succeeded
			...
			// applying channel rastering result (frequency) -----------------------------------
			if (UE->UE_scan_carrier == 1)
			{
	            for (i=0;i<openair0_cfg[UE->rf_map.card].rx_num_channels;i++)
	        	{
	        		if (freq_offset >= 0)
	    			{
	    				downlink_frequency[CC_id][i] = openair0_cfg[UE->rf_map.card].rx_freq[UE->rf_map.chain+i] + abs(UE->common_vars.freq_offset);
	    			}
					else
					{
						downlink_frequency[CC_id][i] = openair0_cfg[UE->rf_map.card].rx_freq[UE->rf_map.chain+i] - abs(UE->common_vars.freq_offset);
		...
		else
		{
		    ...
		    if (UE->UE_scan_carrier == 1)
			{
				//try other frequency (other channel raster) -----------------------------------
				if (ret == -2)
				{
				...
				//if the offset is so far : terminate the synchronization procedure -----------
				...
				LOG_I(PHY, "[initial_sync] trying carrier off %d Hz, rxgain %d (DL %u, UL %u)\n",
	                   ...
	                   downlink_frequency[0][0]+freq_offset,
	                   downlink_frequency[0][0]+uplink_frequency_offset[0][0]+freq_offset );
				config_txRxFreq(UE, downlink_frequency[CC_id], uplink_frequency_offset[CC_id], freq_offset, UE->rx_total_gain_dB);
		    }
	    }// initial_sync=0   

	    AssertFatal ( 0== pthread_mutex_lock(&UE->proc.mutex_synch), "");
	    UE->proc.instance_cnt_synch--; // indicate readiness
	    AssertFatal ( 0== pthread_mutex_unlock(&UE->proc.mutex_synch), "");
    ...
    while (!oai_exit) {
        ...
        if (is_synchronized == 0) {
            if (instance_cnt_synch < 0) {  // we can invoke the synch
				if (UE->rx_offset != 0) //go to the next frame
				{
