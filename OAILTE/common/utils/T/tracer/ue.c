...
//  widget_add_child(g, top_container, line, -1);
  ...
  e->legacy = new_view_textlist(10000, 10, g, text);
	...
	widget_add_child(g, line, col, -1);
	container_set_child_growable(g, line, col, 1);
	widget_add_child(g, col, new_label(g, "PHY, MAC, RLC, PDCP, RRC"), -1);
	text = new_textlist(g, 100, 30, new_color(g, "#afa"));
	widget_add_child(g, col, text, -1);
	container_set_child_growable(g, col, text, 1);
	textview = new_view_textlist(10000, 10, g, text);
	//set the view of all layers to one textview
	e->phyview = textview;
	e->macview = textview;
	e->rlcview = textview;
	e->pdcpview = textview;
	...
	widget_add_child(g, line, col, -1);
	container_set_child_growable(g, line, col, 1);
	widget_add_child(g, col, new_label(g, "Signaling message"), -1);
	text = new_textlist(g, 100, 30, new_color(g, "#aff"));
	widget_add_child(g, col, text, -1);
	container_set_child_growable(g, col, text, 1);
	textview = new_view_textlist(10000, 10, g, text);
	//set the view of all layers to one textview
	e->signaling = textview;
