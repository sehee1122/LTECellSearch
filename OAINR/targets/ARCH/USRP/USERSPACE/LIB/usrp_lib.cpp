 ...
 * \returns 0 in success
 */
int trx_usrp_adjust_rx_freq(openair0_device *device, openair0_config_t *openair0_cfg)
{
	usrp_state_t *s = (usrp_state_t *)device->priv;

	uhd::tune_request_t rx_tune_req(openair0_cfg[0].rx_freq[0] + openair0_cfg[0].digital_freq_offset);
...
int trx_usrp_configure_on(openair0_device *device, openair0_config_t *openair0_cfg)
{
	...
	else if (device->type == USRP_B200_DEV)
	{
		switch ((int)openair0_cfg[0].sample_rate) {
		  	case 30720000:
	...
	for(int i=0; i<s->usrp->get_rx_num_channels(); i++)
	{
		if (i<openair0_cfg[0].rx_num_channels)
		{
		  	...
		  	set_rx_gain_offset(&openair0_cfg[0],i,openair0_cfg[0].bw_gain_adjust);
	...
	while (s->usrp->get_rx_sensor("lo_locked", 0).to_bool() == false)
		usleep(10);	


#if DNF_PWROPTRF
	// recreate tx & rx streamer
	 int samples=openair0_cfg[0].sample_rate;
	 samples/=10000;

	 uhd::stream_args_t stream_args_rx("sc16", "sc16");
	 if ( samples < openair0_cfg[0].max_buff_size )
...
extern "C" {
  ...
  int device_init(openair0_device *device, openair0_config_t *openair0_cfg) {
    ...
    if (u_sf_mode == 2) {
      ...
      device->openair0_cfg = openair0_cfg;
	  device->adjust_rx_freq_func = trx_usrp_adjust_rx_freq;
	  device->trx_configure_on = trx_usrp_configure_on;
	...
	return -1;
      ...
      if (device->type == USRP_B200_DEV) {
		...
		openair0_cfg[0].bw_gain_adjust = bw_gain_adjust;
