...
int nr_ue_pbch_procedures(uint8_t eNB_id,PHY_VARS_NR_UE *ue,UE_nr_rxtx_proc_t *proc, uint8_t abstraction_flag)
{
  int ret = 0;
  ...
  if (ret==0) {
...
#ifdef DEBUG_PHY_PROC
    LOG_D(PHY,"[UE %d] frame %d, nr_tti_rx %d, Received PBCH (MIB):  N_RB_DL %d\n",
    ue->Mod_id,
    frame_rx,
    nr_tti_rx,
    ...
    */
	...
	{
		int rx_offset, f_off;
		uint16_t start_offset = ue->frame_parms.ofdm_symbol_size + ue->frame_parms.nb_prefix_samples0;
				  
		start_meas(&generic_time[TIME_PSS]);
			
		rx_offset = nr_ue_search_online(ue->common_vars.rxdata, ///rx data in time domain
						  nr_tti_rx,
						  start_offset,
						  &(ue->frame_parms),
						  0,
		...
		int duration_us = generic_time[TIME_PSS].p_time/(cpuf*1000.0);
			
		printf("PSS online search duration %4d us pos : %i, sync:%i, freq off : %i\n", duration_us, start_offset+rx_offset, f_off);
...
int phy_procedures_nrUE_RX(PHY_VARS_NR_UE *ue,UE_nr_rxtx_proc_t *proc,uint8_t eNB_id,
  ...
  if ( (nr_tti_rx == ue->ssb_slot_offset) && (ue->decode_MIB == 1))
    {
      int rx_offset, f_off;
	  //uint16_t start_offset = ue->frame_parms.ofdm_symbol_size + ue->frame_parms.nb_prefix_samples0;
	  //ix_halRfic_t* vRfptr = (ix_halRfic_t*)ue->vRfptr;
	  
      LOG_D(PHY," ------  PBCH ChannelComp/LLR: frame.slot %d.%d ------  \n", frame_rx%1024, nr_tti_rx);
      for (int i=0; i<3; i++)
		nr_slot_fep(ue,
			    //(3+i), //mu=0 case A, 3 should be modified
			    (ue->ssb_symbol_offset+1+i),
			    nr_tti_rx,
			    0,
			    0,
			    1,
      ...
      pbch_res = nr_ue_pbch_procedures(eNB_id,ue,proc,0);
	  ...
	  if (rx_offset != -1)
	  {
	  	NHAL_RFcmd_cfgRxDrift(rx_offset + start_offset - 
	  							(ue->frame_parms.ofdm_symbol_size*2 + ue->frame_parms.nb_prefix_samples + ue->frame_parms.nb_prefix_samples0));
	  	NHAL_RFcmd_cfgFreqOffset(f_off + NHAL_RFcmd_readDigitalFreqOffset(), nhal_freqOff_abs);
	  }
	  else
	  {
		LOG_E(PHY, " >>>>> online search failed!!\n");
	  }
	  #endif
