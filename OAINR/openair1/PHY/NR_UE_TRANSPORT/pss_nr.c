...
int nr_ue_search_online(int **rxdata, ///rx data in time domain
								int nr_tti_rx,
		       					...
		       					int *f_off)
{
	...
	int64_t avg;
	double ffo_est=0;
	int srch_offset = nr_tti_rx*(frame_parms->samples_per_slot) + start_offset;
	int pss_index = GET_NID2(frame_parms->Nid_cell);

	unsigned int length = (frame_parms->samples_per_slot)*0.2 ;
	...
	if (fo_flag)
	{
		...
		int64_t result1,result2;
		// Computing cross-correlation at peak on half the symbol size for first half of data
		result1  = dot_product64((short*)primary_synchro_time_nr[pss_index], 
				  (short*) &(rxdata[0][peak_position+srch_offset]), 
				  frame_parms->ofdm_symbol_size>>1, 
				  shift);
		// Computing cross-correlation at peak on half the symbol size for data shifted by half symbol size 
		// as it is real and complex it is necessary to shift by a value equal to symbol size to obtain such shift
		result2  = dot_product64((short*)primary_synchro_time_nr[pss_index]+(frame_parms->ofdm_symbol_size), 
				  (short*) &(rxdata[0][peak_position+srch_offset])+(frame_parms->ofdm_symbol_size), 
				  frame_parms->ofdm_symbol_size>>1, 
				  shift);
  	...
  	*f_off = ffo_est*frame_parms->subcarrier_spacing;  
  	...
  	//LOG_I(PHY,"[UE] online searcher: Peak found at pos %d, val = %llu (%d dB) avg %d dB, ffo %lf\n", peak_position, (unsigned long long)peak_value, dB_fixed64(peak_value),dB_fixed64(avg),ffo_est);
