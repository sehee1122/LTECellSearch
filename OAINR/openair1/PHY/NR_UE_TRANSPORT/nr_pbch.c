...
int nr_rx_pbch( PHY_VARS_NR_UE *ue,
  ...
  if (ue->is_synchronized > 0)
    symbol_offset=ue->ssb_symbol_offset; // 2  should be modified
  else
    symbol_offset=0;
  ...
  write_output("rxdataF0_pbch.m","rxF0pbch",
               &nr_ue_common_vars->common_vars_rx_data_per_thread[ue->current_thread_id[slot_nr]].rxdataF[0][(symbol_offset+1)*frame_parms->ofdm_symbol_size],frame_parms->ofdm_symbol_size*3,1,1);
#endif


  // symbol refers to symbol within SSB. symbol_offset is the offset of the SSB wrt start of slot
  for (symbol=1; symbol<4; symbol++) {
    nr_pbch_extract(nr_ue_common_vars->common_vars_rx_data_per_thread[ue->current_thread_id[slot_nr]].rxdataF,
                    nr_ue_common_vars->common_vars_rx_data_per_thread[ue->current_thread_id[slot_nr]].dl_ch_estimates[eNB_id],
