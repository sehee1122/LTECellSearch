			       ...
			       unsigned char symbol)
{
  ...
  uint8_t nushift, ssb_index=ue->frame_parms.ssb_index, n_hf=ue->frame_parms.n_hf;
  ...
  else dmrss = symbol-(ue->ssb_symbol_offset+1);
  ...
  AssertFatal((symbol > 0 && symbol < 4 && ue->is_synchronized == 0) || 
	      (symbol > 2 && symbol < 6 && ue->is_synchronized == 1),
	      "symbol %d is illegal for PBCH DM-RS (is_synchronized %d)\n",
	      symbol,ue->is_synchronized); //2, 6 should be fixed
#endif
